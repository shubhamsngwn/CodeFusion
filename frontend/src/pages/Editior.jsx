  ...
  useEffect(() => {
    ...
    if (existingRoomId) {
      ...
      socket.on("broadcast code", (code) => {
        setUniqId(code);
        localStorage.setItem("roomId", code); // Save room ID for session
      });
    }
    //NEW CODE ENDS
    ...
    // })
    // if(!isHost) {
    //   socket.emit("join-room", {roomId, uuid, username}) //client
    // }
    ...
    return () => {
      ...
      socket.off("broadcast code"); //NEW CODE
    };
  }, [projectID, htmlCode, cssCode, jsCode]);

  const runOtherLanguages = () => {
    navigate("/otherlanguages");
  };

  return (
    <>
      <EditiorNavbar uniqId={uniqId} />
      <ToastContainer />
      <div className="flex">
          ...
          <div className="tabs flex items-center justify-between gap-2 w-full bg-[#1A1919] h-[50px] px-[40px]">
            <div className="tabs flex items-center gap-2">
              ...
              <button
                ...
                className="tab cursor-pointer p-[6px] bg-blue-600 px-[10px] text-[15px] text-white rounded"
              >
                Run Other Languages
              </button>
            </div>
